...
# --- CONFIGURATION ---
STUDY_NAME = "RMax_Multi_Objective_8x8"
JOURNAL_PATH = f"./{STUDY_NAME}.log"

N_TRIALS = 100
N_JOBS = -1

# The 5 independent seeds used in the QUEST HPO for 8x8
EVAL_SEEDS = [111, 222, 333, 444, 555]
CONVERGENCE_THRESHOLD = 0.8000 # 80% success rate threshold for 8x8
...
BASE_CONFIG = {
    ...
    "max_steps_per_episode": 200,              # Correct 200 steps limit for 8x8
    "render": False,
}
...
def objective(trial):
    ...
    for seed in EVAL_SEEDS:
        ...
        run_name = f"optuna_rmax_8x8/Trial_{trial.number}_Seed_{seed}_{timestamp}"
        
        # Run training loop using the shared main_rmax runner
        ...
        try:
            ...
            # Mean success rate of last 1000 episodes (as in QUEST 8x8 HPO)
            final_perf = df['reward'].tail(1000).mean()
            seed_performances.append(final_perf)
            
            # Episode where rolling 100-episode average reward >= 80%
...
if __name__ == "__main__":
    print(f"Starting Reproducible Multi-Objective Optuna Study on 8x8 for R-Max: {STUDY_NAME}")
    
    # Initialize concurrent storage engine
    storage = JournalStorage(JournalFileStorage(JOURNAL_PATH))
    
    # Seeding the Optuna sampler for reproducibility
