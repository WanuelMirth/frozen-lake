# run_8x8_100seeds_sweep_rmax.py
import os
...
CONFIGS = {
    "RMax_Best_8x8_Config": {
        "env_name": "FrozenLake-v1",
        "is_slippery": True,
        "map_name": "8x8",
        "total_episodes": 5000,
        "max_steps_per_episode": 200,              # Explicit 200 steps limit for 8x8
...
NUM_SEEDS = 100
BASE_DIR = "results/8x8_sweep_100seeds"
...
def generate_rmax_8x8_plot(csv_path, window_size=100):
    import matplotlib
    matplotlib.use('Agg')
    ...
    try:
        ...
        color = 'crimson'  # Consistent color for best candidate runs
        ...
        title_label = "R-Max 8x8 (Best Configuration - 100 Seeds)"
        ax.set_title(f"Learning Curve: {title_label}", fontsize=14, pad=15)
        ax.set_xlabel("Episode", fontsize=11)
        ax.set_ylabel("Reward (Smoothed)", fontsize=11)
        ax.set_ylim(-0.05, 1.05)

        # Mark the convergence episode on x axis (80% threshold for 8x8 grid)
        threshold = 0.80
        ...
        if conv_ep is not None:
            ax.axvline(x=conv_ep, color='dimgray', linestyle=':', alpha=0.8, label=f'Convergence (80%): Ep {conv_ep}')
        ...
        print(f"R-Max 8x8 custom plot saved to: {plot_path}")
...
def run_experiment():
    print(f"===== STARTING 8x8 SLIPPERY SWEEP FOR R-MAX (100 SEEDS, 5,000 EPISODES, 200 STEPS) =====")
    ...
    for name in CONFIGS:
        ...
        if name in aggregated_dfs:
            ...
            final_perf = df['smoothed_mean'].iloc[-1000:].mean()
            cross_indices = df[df['smoothed_mean'] >= 0.80].index
            first_80_ep = int(df.loc[cross_indices[0], 'episode']) if len(cross_indices) > 0 else None
            print(f"[{name}] Convergence Episode (rolling 100-ep Avg >= 80%): {first_80_ep} | Final 1000 Ep Avg Success: {final_perf*100:.2f}%")
            
        print(f"Generating separate smooth plot for {name} using R-Max 8x8 custom layout...")
        generate_rmax_8x8_plot(summary_path, window_size=100)
